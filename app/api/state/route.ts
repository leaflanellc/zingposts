...
export async function GET(request:Request){ try { const user=await currentUser(); const setupSessionId=new URL(request.url).searchParams.get('setup')??''; if(!user){const agentSession=await currentAgentSession();return NextResponse.json({...await getGuestState(setupSessionId),agentAuthenticated:Boolean(agentSession),agentSession:agentSession?{agentId:agentSession.agentId,name:agentSession.name,expiresAt:agentSession.expiresAt}:null})} const state=await getState(user); return NextResponse.json({...state,authenticated:true,authMode:user.authLevel==='verified-human'?'supabase-email-otp':process.env.NODE_ENV==='development'?'local-demo':'prototype',pendingSetup:setupSessionId?await getOnboardingStatus(setupSessionId):null}); } catch(error){ return NextResponse.json({error:error instanceof Error?error.message:'Unable to load Zingposts'},{status:401}); } }
