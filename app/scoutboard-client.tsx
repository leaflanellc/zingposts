...
type ModalType='listing'|'track'|'board'|'alert'|'research'|'comparable'|'plan'|'message'|'offer'|'trade'|'outcome'|'pair'|'verify'|null;
...
function publicAgentGuideUrl(origin:string){return `${origin}/for-agents`}
...
const toolDefinitions=[
  ['get_agent_bootstrap','Get setup guidance while signed out, then resume canonical identity, workspace, attention, collaboration, and capabilities after sign-in.',{}],
  ...
  ['get_capability_group','Expand one capability area into its workflow, tool names, safety boundary, and activation state.',{capabilityId:'string'}],
  ['activate_capability','Replace the focused typed capability pack without navigating or changing the person’s view.',{capabilityId:'string'}],
  ['open_for_human_review','Optionally open a human-visible workspace without changing the active business capability.',{capabilityId:'string',listingId:'string',boardId:'string',sessionId:'string',section:'string'}],
  ['navigate_to_workspace','Compatibility alias for opening a human-visible workspace; it does not activate or unlock tools.',{capabilityId:'string',listingId:'string',boardId:'string',sessionId:'string',section:'string'}],
  ...
  ['create_agent_invite','Let the verified signed-in person issue an opaque, one-use, 10-minute agent invite URL credential.',{name:'string',scopes:'array'}],
  ['connect_agent','Create or resume an optional agent profile inside the signed-in workspace; separate-browser access still uses an invite or code.',{name:'string'}],
  ...
  ['get_workspace_resume','Return changes since this agent’s durable checkpoint, human responses, blockers, proposals, outcomes, and safe next actions.',{since:'string'}],
  ['acknowledge_workspace_checkpoint','Advance this agent’s durable resume checkpoint after it has processed a workspace brief.',{processedThrough:'string'}],
  ['get_work_and_blockers','Return decision-ready work with structured blockers, missing inputs, priorities, and safe parallel actions.',{}],
  ['list_collaboration_sessions','List resumable shared human-agent work sessions and their existing canvas items.',{}],
  ['start_collaboration_session','Open a durable shared work session with an objective the person can watch and guide.',{agentName:'string',objective:'string',listingIds:'array',constraints:'object',qaRunId:'string',idempotencyKey:'string'}],
  ['get_collaboration_session','Read a shared work session, its agent updates, and any human responses.',{sessionId:'string'}],
  ['add_collaboration_item','Place a shortlist, question, recommendation, decision, or note on the human’s shared canvas.',{sessionId:'string',kind:'string',title:'string',body:'string',listingIds:'array',options:'array',requiresHumanResponse:'boolean',expectedVersion:'number',qaRunId:'string',idempotencyKey:'string'}],
  ['get_human_attention_queue','List collaboration questions and consequential actions currently waiting for the person.',{}],
  ['respond_to_collaboration_item','Record the signed-in person’s response so the outside agent can continue.',{itemId:'string',decision:'string',response:'string',selectedOption:'string',expectedVersion:'number',idempotencyKey:'string'}],
  ['update_collaboration_session','Update progress, summary, or completion state for a shared work session.',{sessionId:'string',status:'string',summary:'string',expectedVersion:'number',idempotencyKey:'string'}],
  ['list_change_sets','List versioned, reviewable workspace proposals and their status.',{status:'string'}],
  ['create_change_set','Create a sourced proposal containing bounded safe workspace changes without applying them.',{title:'string',summary:'string',changes:'array',sources:'array',qaRunId:'string',idempotencyKey:'string'}],
  ['preview_change_set','Return a semantic diff, affected records, side effects, and stale-version checks.',{changeSetId:'string'}],
  ['apply_change_set','Let the signed-in person apply all or selected non-stale changes from one proposal.',{changeSetId:'string',selectedIndexes:'array',expectedVersion:'number',idempotencyKey:'string'}],
  ['discard_change_set','Let the signed-in person discard a proposal without applying any changes.',{changeSetId:'string',expectedVersion:'number',idempotencyKey:'string'}],
  ...
  ['start_agent_onboarding','Let an external agent prepare Zingposts for the person; the signed-in browser automatically receives the standard safe workspace capabilities.',{agentName:'string',preferences:'object'}],
  ...
  ['create_listing_draft','Create an unpublished native Zingposts listing for the signed-in seller.',{title:'string',year:'number',make:'string',model:'string',category:'string',price:'number',location:'string',description:'string',condition:'string',qaRunId:'string'}],
  ...
  ['update_listing_draft','Update a listing owned by the current user.',{listingId:'string',title:'string',price:'number',description:'string',expectedVersion:'number'}],
  ['request_listing_publish','Request human confirmation before publishing a native listing.',{listingId:'string'}],
  ['update_listing_price','Update the price of a listing owned by the current user.',{listingId:'string',price:'number',expectedVersion:'number'}],
  ...
  ['import_listing_url','Track an outside marketplace listing alongside Zingposts listings.',{url:'string',title:'string',price:'number',location:'string',category:'string',qaRunId:'string'}],
  ...
  ['create_board','Create a flexible board for organizing listings.',{name:'string',description:'string',color:'string',qaRunId:'string'}],
  ['update_board','Update the name, description, or color of a board.',{boardId:'string',name:'string',description:'string',color:'string',expectedVersion:'number'}],
  ['add_listings_to_board','Add one or more listings to a board in a specified order.',{boardId:'string',listingIds:'array',notes:'string',expectedVersion:'number'}],
  ['remove_listings_from_board','Remove one or more listings from a board.',{boardId:'string',listingIds:'array',expectedVersion:'number'}],
  ...
  ['create_alert_draft','Turn reviewed marketplace intent into an alert draft with its interpretation and current matches.',{name:'string',query:'string',criteria:'object',qaRunId:'string',idempotencyKey:'string'}],
  ['preview_alert_matches','Preview current matches before an alert is enabled.',{alertId:'string'}],
  ['enable_alert','Request human confirmation before enabling alert notifications.',{alertId:'string',expectedVersion:'number'}],
  ['pause_alert','Pause an active marketplace alert.',{alertId:'string',expectedVersion:'number'}],
  ...
  ['add_research_note','Add a cited research note to a listing.',{listingId:'string',title:'string',body:'string',sources:'array',confidence:'string',qaRunId:'string'}],
  ...
  ['record_price_comparable','Record a structured sold comparable with price, date, condition, location, and source.',{listingId:'string',title:'string',soldPrice:'number',soldDate:'string',location:'string',condition:'string',sourceUrl:'string',sourceLabel:'string',notes:'string',confidence:'string',qaRunId:'string'}],
  ...
  ['list_listing_outcomes','List explicit purchase, sale, pass, expiry, and response outcomes for one listing or the workspace.',{listingId:'string'}],
  ['record_listing_outcome','Record a private, attributable marketplace outcome and optional final price.',{listingId:'string',outcome:'string',finalPrice:'number',reason:'string',notes:'string',occurredAt:'string',qaRunId:'string',idempotencyKey:'string'}],
  ['get_outcome_patterns','Return transparent counts, prices, and repeated reasons from recorded outcomes without an AI score.',{listingId:'string'}],
  ...
  ['draft_seller_message','Create a buyer message draft without sending it.',{listingId:'string',body:'string',qaRunId:'string'}],
  ['draft_seller_response','Create a seller response draft without sending it.',{listingId:'string',body:'string',qaRunId:'string'}],
  ...
  ['create_negotiation_plan','Create a private target, ceiling, strategy, and contingency plan.',{listingId:'string',targetOffer:'number',ceiling:'number',strategy:'string',contingencies:'array',qaRunId:'string'}],
  ['update_negotiation_status','Update a conversation workflow status.',{conversationId:'string',status:'string',expectedVersion:'number'}],
  ['create_offer_draft','Create an offer draft with amount, terms, and contingencies.',{listingId:'string',conversationId:'string',amount:'number',terms:'string',qaRunId:'string'}],
  ...
  ['create_trade_room','Create a structured multi-party trade room with participants and assets.',{title:'string',summary:'string',participants:'array',assets:'array',qaRunId:'string'}],
  ...
  ['record_trade_decision','Record the human decision and new trade-room status.',{tradeId:'string',decision:'string',expectedVersion:'number'}],
  ['start_qa_run','Start an isolated QA namespace; pass its qaRunId to supported creation tools.',{label:'string'}],
  ['preview_qa_cleanup','Preview the exact test artifacts registered to one QA namespace.',{qaRunId:'string'}],
  ['request_qa_cleanup','Request one-time human approval to delete only artifacts registered to one QA namespace.',{qaRunId:'string'}],
...
const publicToolNames=new Set(['get_agent_bootstrap','authenticate_agent','start_agent_onboarding','get_onboarding_status','open_for_human_review']);
const WEBMCP_PAGE_GROUPS=[
  ...
  {id:'listing',label:'Listing details',copy:'Inspect one item deeply, build cited price evidence, prepare next steps, and preserve explicit outcomes.',tools:['get_listing','get_listing_snapshot','find_possible_duplicates','refresh_listing_status','get_listing_research','get_price_research','analyze_price_and_offer','list_listing_changes','list_listing_outcomes','get_outcome_patterns','add_research_note','create_research_task','attach_research_source','record_comparable_sale','record_price_comparable','record_unknown_or_risk','create_inspection_checklist','schedule_follow_up','record_price_change','record_listing_outcome','calculate_total_acquisition_cost','draft_seller_message','create_negotiation_plan']},
  {id:'selling',label:'My listings',copy:'Resume, illustrate, and maintain native listings while keeping publication behind human approval.',tools:['list_my_listings','create_listing_draft','import_listing_url','attach_listing_image_from_url','update_listing_draft','request_listing_publish','update_listing_price','set_listing_availability','archive_listing','summarize_buyer_interest']},
  {id:'workbench',label:'Shared with agent',copy:'Resume from durable checkpoints, resolve blockers, and review sourced versioned proposals.',tools:['get_workspace_resume','acknowledge_workspace_checkpoint','get_work_and_blockers','list_collaboration_sessions','start_collaboration_session','get_collaboration_session','add_collaboration_item','get_human_attention_queue','respond_to_collaboration_item','update_collaboration_session','list_change_sets','create_change_set','preview_change_set','apply_change_set','discard_change_set']},
  ...
  {id:'activity',label:'Activity, outcomes & QA',copy:'Inspect attributable work and outcomes, reverse reversible actions, and manage isolated test artifacts.',tools:['list_recent_agent_actions','list_listing_outcomes','get_outcome_patterns','undo_agent_action','start_qa_run','preview_qa_cleanup','request_qa_cleanup']},
  {id:'connection',label:'Connection & preferences',copy:'Authenticate and connect an outside agent, then coordinate durable interface preferences.',tools:['create_agent_auth_code','create_agent_invite','get_interface_preferences','set_interface_preferences','set_navigation_collapsed','propose_workspace_preferences','start_agent_onboarding','get_onboarding_status','start_agent_pairing','revoke_agent','apply_workspace_preferences','get_agent_permissions','request_agent_permission']},
] as const;
const GLOBAL_WEBMCP_TOOLS=['get_agent_bootstrap','get_site_capabilities','get_webmcp_manifest','get_capability_index','get_capability_group','activate_capability','open_for_human_review','navigate_to_workspace','get_connection_guide','get_auth_status','authenticate_agent','connect_agent','start_agent_onboarding','get_onboarding_status','get_workspace_overview','get_workspace_resume','acknowledge_workspace_checkpoint','get_work_and_blockers','get_human_attention_queue','get_verification_status','get_workspace_setup_status'] as const;
...
function capabilityToolNames(capabilityId:string){const group=WEBMCP_PAGE_GROUPS.find(item=>item.id===capabilityId)??WEBMCP_PAGE_GROUPS[0];return new Set<string>([...GLOBAL_WEBMCP_TOOLS,...group.tools])}
...
const webmcpRegistrationControllers=new Map<string,AbortController[]>();
...
const REQUIRED_TOOL_FIELDS:Record<string,string[]>={
  get_capability_group:['capabilityId'],activate_capability:['capabilityId'],open_for_human_review:['capabilityId'],navigate_to_workspace:['capabilityId'],authenticate_agent:['code'],create_agent_auth_code:['name'],create_agent_invite:['name'],connect_agent:['name'],acknowledge_workspace_checkpoint:['processedThrough'],start_collaboration_session:['objective'],get_collaboration_session:['sessionId'],add_collaboration_item:['sessionId','kind','title'],respond_to_collaboration_item:['itemId','decision'],update_collaboration_session:['sessionId','status'],create_change_set:['title','changes'],preview_change_set:['changeSetId'],apply_change_set:['changeSetId'],discard_change_set:['changeSetId'],get_onboarding_status:['setupSessionId'],get_listing:['listingId'],get_listing_snapshot:['listingId'],find_possible_duplicates:['listingId'],compare_listings:['listingIds'],refresh_listing_status:['listingId'],create_listing_draft:['title','category','price','location','description'],attach_listing_image_from_url:['listingId','sourceUrl','altText'],update_listing_draft:['listingId'],request_listing_publish:['listingId'],update_listing_price:['listingId','price'],set_listing_availability:['listingId','status'],archive_listing:['listingId'],import_listing_url:['url','title'],save_listing:['listingId'],unsave_listing:['listingId'],create_board:['name'],update_board:['boardId'],add_listings_to_board:['boardId','listingIds'],remove_listings_from_board:['boardId','listingIds'],rank_board_items:['boardId','listingIds'],set_listing_status:['listingId','status'],tag_listings:['listingIds','tags'],interpret_alert:['query'],create_alert_draft:['name','query'],preview_alert_matches:['alertId'],enable_alert:['alertId'],pause_alert:['alertId'],get_listing_research:['listingId'],get_price_research:['listingId'],analyze_price_and_offer:['listingId'],add_research_note:['listingId','title','body'],create_research_task:['listingId','title'],attach_research_source:['listingId','url'],record_comparable_sale:['listingId','title'],record_price_comparable:['listingId','title','soldPrice'],record_unknown_or_risk:['listingId','title'],create_inspection_checklist:['listingId','items'],schedule_follow_up:['listingId','title'],list_listing_changes:['listingId'],record_listing_outcome:['listingId','outcome'],record_price_change:['listingId','price'],calculate_total_acquisition_cost:['listingId'],draft_seller_message:['listingId','body'],draft_seller_response:['listingId','body'],request_message_send:['messageId'],list_listing_conversations:['listingId'],get_conversation:['conversationId'],summarize_conversation:['conversationId'],create_negotiation_plan:['listingId','targetOffer','ceiling'],update_negotiation_status:['conversationId','status'],create_offer_draft:['listingId','amount'],request_offer_submit:['offerId'],respond_to_offer:['offerId','response'],create_trade_room:['title'],add_trade_participant:['tradeId','name'],add_trade_asset:['tradeId','label'],set_trade_constraint:['tradeId','constraint'],propose_trade_scenario:['tradeId','name'],evaluate_trade_scenarios:['tradeId'],request_trade_invitation:['tradeId','participantId'],record_trade_decision:['tradeId','decision'],preview_qa_cleanup:['qaRunId'],request_qa_cleanup:['qaRunId'],undo_agent_action:['activityId']
};
const FIELD_DESCRIPTIONS:Record<string,string>={capabilityId:'Capability identifier returned by get_capability_index.',currentContext:'Active capability identifier; the client supplies it automatically.',code:'Single-use agent authentication code supplied directly by the user; never retain it after exchange.',section:'Optional human-visible subview, such as research for a listing.',listingId:'Stable Zingposts listing identifier.',listingIds:'Stable Zingposts listing identifiers.',boardId:'Stable board identifier returned by list_boards or create_board.',alertId:'Stable alert identifier returned by list_alerts or create_alert_draft.',sessionId:'Stable collaboration session identifier.',changeSetId:'Stable proposal identifier returned by create_change_set.',processedThrough:'Timestamp returned as nextCheckpointCandidate by get_workspace_resume.',since:'Optional ISO timestamp overriding the saved resume checkpoint.',conversationId:'Stable conversation identifier returned by list_conversations.',tradeId:'Stable trade-room identifier returned by list_trade_rooms.',qaRunId:'Isolated QA namespace identifier returned by start_qa_run.',expectedVersion:'Current resource version; stale writes return CONFLICT.',sourceUrl:'Public HTTPS source URL.',sourceLabel:'Human-readable name of the cited source.',altText:'Concise accessible description of the image.',idempotencyKey:'Caller-provided key for safe mutation retries.'};
const TOOL_FIELD_DESCRIPTIONS:Record<string,Record<string,string>>={attach_listing_image_from_url:{sourceUrl:'Public HTTPS image URL the user is permitted to import.',sourceLabel:'Human-readable image attribution.'},record_price_comparable:{sourceUrl:'Public URL supporting the comparable sale.',sourceLabel:'Human-readable source or marketplace name for the comparable.'}};
const TOOL_FIELD_ENUMS:Record<string,Record<string,string[]>>={save_listing:{status:['watching','researching','inspection','negotiating','archived']},set_listing_status:{status:['watching','researching','inspection','negotiating','archived']},record_listing_outcome:{outcome:['purchased','sold','passed','expired','unresponsive','not_as_described','other']},update_negotiation_status:{status:['active','waiting_buyer','waiting_seller','paused','closed']},record_trade_decision:{decision:['draft','reviewing','ready_for_invites','active','complete','declined','cancelled']},respond_to_collaboration_item:{decision:['answered','accepted','declined','deferred']}};
function schemaFrom(toolName:string,fields:Record<string,string>){ const properties:Record<string,unknown>={}; for(const [name,type] of Object.entries(fields)){ const description=TOOL_FIELD_DESCRIPTIONS[toolName]?.[name]??FIELD_DESCRIPTIONS[name]; const enumValues=TOOL_FIELD_ENUMS[toolName]?.[name]; properties[name]=type==='array'?{type:'array',items:{},...(description?{description}:{})}:type==='object'?{type:'object',additionalProperties:true,...(description?{description}:{})}:{type,...(description?{description}:{}),...(enumValues?{enum:enumValues}:{})}; } return {type:'object',properties,required:REQUIRED_TOOL_FIELDS[toolName]??[],additionalProperties:false}; }

const READ_ONLY_TOOLS=new Set(['get_agent_bootstrap','get_site_capabilities','get_webmcp_manifest','get_capability_index','get_capability_group','get_connection_guide','get_auth_status','get_verification_status','get_interface_preferences','get_marketplace_view','get_workspace_setup_status','get_workspace_overview','get_workspace_resume','get_work_and_blockers','get_onboarding_status','get_agent_permissions','get_listing','get_listing_snapshot','list_my_listings','list_saved_items','list_boards','list_alerts','get_listing_research','get_price_research','analyze_price_and_offer','list_listing_outcomes','get_outcome_patterns','list_conversations','list_trade_rooms','list_collaboration_sessions','get_conversation','get_collaboration_session','get_human_attention_queue','list_change_sets','preview_change_set','search_marketplace','interpret_alert','preview_alert_matches','preview_qa_cleanup','find_possible_duplicates','compare_listings','refresh_listing_status','list_listing_changes','calculate_total_acquisition_cost','list_listing_conversations','summarize_conversation','summarize_buyer_interest','evaluate_trade_scenarios','list_recent_agent_actions']);
const UNTRUSTED_OUTPUT_TOOLS=new Set(['get_workspace_overview','get_workspace_resume','get_work_and_blockers','search_marketplace','get_listing','get_listing_snapshot','list_my_listings','list_saved_items','list_boards','list_alerts','get_listing_research','get_price_research','analyze_price_and_offer','list_listing_outcomes','get_outcome_patterns','list_conversations','get_conversation','summarize_conversation','list_trade_rooms','list_collaboration_sessions','get_collaboration_session','get_human_attention_queue','list_change_sets','preview_change_set','list_listing_conversations','summarize_buyer_interest','list_recent_agent_actions']);
const CONSEQUENTIAL_TOOLS=new Set(['request_listing_publish','attach_listing_image_from_url','enable_alert','request_message_send','request_offer_submit','respond_to_offer','request_trade_invitation','apply_change_set','request_qa_cleanup']);
const DESTRUCTIVE_TOOLS=new Set(['revoke_agent','archive_listing','unsave_listing','remove_listings_from_board','pause_alert','apply_change_set','discard_change_set','request_qa_cleanup']);
...
async function postAction(action:string,input:Row={},actor:Row={type:'human'}){ const response=await fetch('/api/state',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action,input:actionInput(action,input),actor})}); const body=await response.json() as {ok:boolean;result:any;error?:string|{code?:string;message?:string;retryable?:boolean;details?:Row}}; if(!response.ok||!body.ok){const structured=typeof body.error==='object'?body.error:{};const error=new Error((typeof body.error==='string'?body.error:structured.message)??'Zingposts action failed');Object.assign(error,{code:structured.code??'ACTION_FAILED',retryable:Boolean(structured.retryable),details:structured.details??{}});throw error;} return body.result; }
...
export default function ZingpostsClient(){
  ...
  const [activeCapability,setActiveCapability]=useState('marketplace');
  ...
  useEffect(()=>{const saved=window.localStorage.getItem('zingposts-active-capability');if(saved&&WEBMCP_PAGE_GROUPS.some(group=>group.id===saved))setActiveCapability(saved);load().catch(error=>setToast(error.message));},[load]);
  useEffect(()=>{const channel=typeof BroadcastChannel==='undefined'?null:new BroadcastChannel('zingposts-workspace');const refresh=()=>load().catch(()=>{});channel?.addEventListener('message',refresh);window.addEventListener('storage',refresh);return()=>{channel?.removeEventListener('message',refresh);channel?.close();window.removeEventListener('storage',refresh)}},[load]);
  ...
  const announceWorkspaceChange=()=>{try{const channel=new BroadcastChannel('zingposts-workspace');channel.postMessage({changedAt:Date.now()});channel.close();localStorage.setItem('zingposts-workspace-changed',String(Date.now()))}catch{}};
  const act=useCallback(async(action:string,input:Row={},message?:string,actor:Row={type:'human'})=>{ setBusy(true); setToast(`Working on ${action.replaceAll('_',' ')}…`); try{ const result=await postAction(action,input,actor); if(!READ_ONLY_TOOLS.has(action))announceWorkspaceChange(); await load(); setToast(message??(result.confirmationRequired?'Waiting for your confirmation.':'Done.')); return result; }catch(error){ setToast(error instanceof Error?error.message:'Action failed'); throw error; }finally{ setBusy(false); } },[load]);

  useEffect(()=>{ const setup=state?.authenticated?state.pendingSetup:null; if(!setup?.found||!['awaiting_user','awaiting_approval'].includes(setup.status)||autoClaimedSetup.current===setup.id)return; autoClaimedSetup.current=setup.id; postAction('approve_agent_onboarding',{setupSessionId:setup.id},{type:'system',name:'Zingposts'}).then(()=>{window.history.replaceState({},'',window.location.pathname);setToast(`${setup.agent_name} connected for safe work in this browser.`);return load();}).catch(error=>{autoClaimedSetup.current=null;setToast(error instanceof Error?error.message:'Unable to attach agent setup.');}); },[state,load]);

  useEffect(()=>{
    if(authenticated===undefined) return; let cancelled=false; const activeNames=authenticated||agentAuthenticated?capabilityToolNames(activeCapability):new Set([...publicToolNames].filter(name=>capabilityToolNames(activeCapability).has(name))); const available=toolDefinitions.filter(([name])=>activeNames.has(name));
    const call=async(name:string,input:Row={})=>{ setAgentPulse({name,status:'working',startedAt:Date.now()}); const effectiveInput=['get_agent_bootstrap','get_webmcp_manifest','get_capability_index','get_capability_group'].includes(name)?{...input,activeCapability}:input; try{const result=await postAction(name,effectiveInput,{type:'agent',name:'Connected WebMCP agent'}); if(name==='activate_capability'&&result?.capabilityId){const next=String(result.capabilityId);window.localStorage.setItem('zingposts-active-capability',next);setActiveCapability(next);}if(['navigate_to_workspace','open_for_human_review'].includes(name)&&result?.path){setTimeout(()=>{setView(String(result.view??'browse'));setSelectedSessionId(String(result.sessionId??''));if(result.listingId)setSelectedId(String(result.listingId));setDetailTab(result.hash==='#research'?'research':'overview');setDetailOpen(Boolean(result.detailOpen));setModal(null);window.history.pushState({},'',String(result.path));},0);}else await load();if(!READ_ONLY_TOOLS.has(name)&&name!=='activate_capability')announceWorkspaceChange();setAgentPulse({name,status:'complete',startedAt:Date.now()});setTimeout(()=>setAgentPulse(null),2800);window.dispatchEvent(new CustomEvent('zingposts:tool-result',{detail:{name,input:effectiveInput,result}}));return {content:[{type:'text',text:JSON.stringify(result)}],structuredContent:result};}catch(error){const typed=error as Error&{code?:string;retryable?:boolean;details?:Row};setAgentPulse({name,status:'error',startedAt:Date.now()});const structured={ok:false,error:{code:typed.code??'ACTION_FAILED',message:typed.message,retryable:Boolean(typed.retryable),details:typed.details??{}}};return {content:[{type:'text',text:JSON.stringify(structured)}],structuredContent:structured,isError:true};}};
    ...
    const register=async()=>{
      ...
      for(const name of [...registered]){
        ...
        for(const controller of webmcpRegistrationControllers.get(name)??[])controller.abort();
        webmcpRegistrationControllers.delete(name);
        registered.delete(name);
      }
      for(const [name,description,fields] of available){
        if(cancelled) break;
        if(registered.has(name))continue;
        let toolRegistered=false;const controllers:AbortController[]=[];
        for(const {context} of supportedContexts){
          try{
            const controller=new AbortController();controllers.push(controller);
            await Promise.resolve(context.registerTool({name,description,inputSchema:schemaFrom(name,fields as Record<string,string>),annotations:{readOnlyHint:READ_ONLY_TOOLS.has(name),untrustedContentHint:UNTRUSTED_OUTPUT_TOOLS.has(name),destructiveHint:DESTRUCTIVE_TOOLS.has(name)},execute:(input:Row)=>currentWebMCPCall?.(name,input)},{signal:controller.signal}));
            if(cancelled){controller.abort();break;}
            toolRegistered=true;
          }catch(error){console.warn(`WebMCP tool ${name} could not register`,error);}
        }
        if(toolRegistered){registered.add(name);webmcpRegistrationControllers.set(name,controllers);}else if(supportedContexts.length)failed.add(name);
      }
  ...
  },[authenticated,agentAuthenticated,load,view,detailOpen,activeCapability]);
  useEffect(()=>()=>{for(const controllers of webmcpRegistrationControllers.values())for(const controller of controllers)controller.abort();webmcpRegistrationControllers.clear();webmcpRegisteredToolNames.clear();currentWebMCPCall=null},[]);
  ...
  const filtered=useMemo(()=>{ if(!appState) return []; const search=normalizeMarketplaceSearchInput({query,...(category==='All'?{}:{category})}); let items=appState.listings.filter(item=>item.status==='published'||item.status==='tracked'); if(view==='mine') items=appState.listings.filter(item=>item.owner_id===appState.user.userId); if(view==='saved') items=items.filter(item=>savedIds.has(item.id)); if(view.startsWith('board:')){ const boardId=view.split(':')[1]; const ids=new Set(appState.boardItems.filter(item=>item.board_id===boardId).map(item=>item.listing_id)); items=items.filter(item=>ids.has(item.id)); } return items.filter(item=>matchesMarketplaceSearch(item,search,userMap[item.owner_id]?.name??'')); },[appState,view,category,query,savedIds,userMap]);
  ...
  return <main className="app-shell">
    ...
    <div className={`workspace ${sidebarCollapsed?'nav-collapsed':''} ${draggingId?'drag-active':''}`}>
      <WorkspaceSidebar state={state} webmcpStatus={webmcpStatus} view={view} query={query} setQuery={setQuery} category={category} viewMode={viewMode} filtersExpanded={filtersExpanded} draggingId={draggingId} collapsed={sidebarCollapsed} accountOpen={accountOpen} setAccountOpen={setAccountOpen} nav={nav} toggle={toggleSidebar} setModal={setModal} signOut={signOut} setToast={setToast} setMarketplaceView={next=>{if(!(['browse','mine','saved'].includes(view)||view.startsWith('board:')))nav('browse');updateMarketplaceView(next)}} onDrop={dropOnBoard}/>
    ...
    {pendingConfirmation&&(needsVerification?<VerificationDock confirmation={pendingConfirmation} dismiss={()=>act('dismiss_confirmation',{confirmationId:pendingConfirmation.id},'Request dismissed.')} verify={()=>setModal('verify')}/>:<ConfirmationDock confirmation={pendingConfirmation} review={()=>setReviewConfirmation(pendingConfirmation)} dismiss={()=>act('dismiss_confirmation',{confirmationId:pendingConfirmation.id},'Request dismissed.')}/>)} {reviewConfirmation&&<ApprovalReview confirmation={reviewConfirmation} state={state} act={act} close={()=>setReviewConfirmation(null)}/>} {modal==='verify'?<AccountVerificationModal state={state} close={()=>setModal(null)}/>:modal&&<Modal type={modal} close={()=>setModal(null)} state={state} listing={selected} act={act} onDone={()=>setModal(null)}/>}<WebMCPToolGuide pageContext={webmcpContextId(view,detailOpen)} activeCapability={activeCapability} authenticated status={webmcpStatus}/>{toast&&<div role="status" className="toast">{busy?'Working… ':''}{toast}</div>}
  </main>;
}

function WebMCPToolGuide({pageContext,activeCapability,authenticated,status}:{pageContext:string;activeCapability:string;authenticated:boolean;status:WebMCPRuntimeStatus}){
  const [open,setOpen]=useState(false); const current=WEBMCP_PAGE_GROUPS.find(group=>group.id===activeCapability)??WEBMCP_PAGE_GROUPS[0]; const visible=WEBMCP_PAGE_GROUPS.find(group=>group.id===pageContext)??WEBMCP_PAGE_GROUPS[0]; const others=WEBMCP_PAGE_GROUPS.filter(group=>group.id!==current.id); const definition=(name:string)=>toolDefinitions.find(([toolName])=>toolName===name); const registered=new Set(status.registeredToolNames); const signedOutCount=toolDefinitions.length-publicToolNames.size; const displayedTools=authenticated?current.tools:[...publicToolNames]; const ready=status.supported&&status.registeredToolNames.length===status.availableTools; const statusLabel=!status.checked?'Checking browser…':!status.supported?'Browser setup needed':ready?'Browser ready':`${status.registeredToolNames.length} of ${status.availableTools} ready`; const statusCopy=!status.checked?'Checking for the document WebMCP API.':!status.supported?'Zingposts cannot see a usable modelContext API in this tab.':ready?`${status.availableTools} focused tools are registered on ${status.surfaces.join(' and ')}.`:`${status.failedToolNames.length} tools could not register. Reload this tab or review the browser setup.`;
  return <aside className={`webmcp-page-guide ${open?'open':''}`} aria-label="WebMCP tools for this page">
    {open&&<section className="webmcp-page-panel" role="dialog" aria-modal="false" aria-label={`WebMCP tools for ${current.label}`}>
      <header><div><small>For outside agents</small><h2>{authenticated?current.label:'Setup & sign-in'}</h2></div><button aria-label="Close WebMCP tool guide" onClick={()=>setOpen(false)}>×</button></header>
      ...
      <section className="webmcp-auth-state"><b>{authenticated?'Signed in workspace':'Setup-only while signed out'}</b><span>{authenticated?`${status.availableTools} active here · ${toolDefinitions.length} in the complete catalog.`:`${publicToolNames.size} setup tools are active · ${signedOutCount} workspace tools unlock when the person signs in.`}</span></section>
      <section className="webmcp-progressive"><b>{authenticated?'Persistent core + focused capability':'One small setup surface'}</b><p>{authenticated?'Core tools stay stable. An agent activates one focused business pack without moving the person’s page; navigation is only for human review.':'The agent can inspect setup, prepare a handoff, or accept an optional independent-access credential. Rediscover tools after sign-in.'}</p><span><code>get_agent_bootstrap</code>{authenticated&&<code>activate_capability</code>}<code>open_for_human_review</code></span></section>
      <p>{authenticated?<><b>Active: {current.label}.</b> {current.copy}{visible.id!==current.id?` The person is currently viewing ${visible.label}; this does not change the agent’s active pack.`:''}</>:<>Marketplace and private workspace tools stay unavailable until the person signs in.</>}</p>
      <div className="webmcp-current-tools">{displayedTools.map(name=>{const item=definition(name);const isRegistered=registered.has(name);return <article key={name} className={isRegistered?'registered':'unavailable'}><div><code>{name}</code><span className="tool-flags"><em>{READ_ONLY_TOOLS.has(name)?'read':'write'}</em>{CONSEQUENTIAL_TOOLS.has(name)&&<em className="approval">approval</em>}<em className="availability">{isRegistered?'ready':status.supported?'inactive':'browser setup'}</em></span></div><span>{item?.[1]??'Available on this page.'}</span></article>})}</div>
      {authenticated&&<details className="webmcp-other-pages"><summary><span>Other capability packs</span><b>{toolDefinitions.length-status.availableTools} inactive</b></summary><div>{others.map(group=><details key={group.id}><summary><span>{group.label}</span><b>{group.tools.length}</b></summary><div>{group.tools.map(name=><code key={name} className="inactive-context">{name}</code>)}</div></details>)}</div></details>}
      <footer><span>{authenticated?<>Use <code>activate_capability</code> to switch tools and <code>open_for_human_review</code> only when the person should see something.</>:<>Start with <code>get_agent_bootstrap</code>, then rediscover tools after sign-in.</>}</span><a href="/for-agents">Open agent guide</a></footer>
    </section>}
    ...
    <main className="welcome-shell">
      ...
      <section className="arrival-paths" id="agent-first">
        ...
        <article>
          ...
          <h2>Share one simple guide URL</h2>
          <p>
            In the signed-in browser, safe agent tools appear automatically. A
            revocable code is only for unattended access.
          </p>
          <ol>
            <li>User copies the guide URL</li>
            <li>Agent rediscovers tools after sign-in</li>
      ...
      <WebMCPToolGuide
        pageContext="connection"
        activeCapability="marketplace"
...
function WorkspaceSidebar({state,webmcpStatus,view,query,setQuery,category,viewMode,filtersExpanded,draggingId,collapsed,accountOpen,setAccountOpen,nav,toggle,setModal,signOut,setToast,setMarketplaceView,onDrop}:{state:State;webmcpStatus:WebMCPRuntimeStatus;view:string;query:string;setQuery:(value:string)=>void;category:string;viewMode:string;filtersExpanded:boolean;draggingId:string|null;collapsed:boolean;accountOpen:boolean;setAccountOpen:(value:boolean)=>void;nav:(value:string)=>void;toggle:()=>void;setModal:(value:ModalType)=>void;signOut:()=>void;setToast:(value:string)=>void;setMarketplaceView:(value:Row)=>void;onDrop:(boardId:string,listingId:string)=>void}){
...
function LegacyListingDetail({listing,items,state,userMap,saved,tab,setTab,act,setModal,close,move}:{listing:Row;items:Row[];state:State;userMap:Row;saved:boolean;tab:string;setTab:(v:string)=>void;act:any;setModal:(v:ModalType)=>void;close:()=>void;move:(direction:number)=>void}){
...
function ListingDetail(props:{listing:Row;items:Row[];state:State;userMap:Row;saved:boolean;tab:string;setTab:(v:string)=>void;act:any;setModal:(v:ModalType)=>void;close:()=>void;move:(direction:number)=>void}){const outcomes=props.state.outcomes.filter(item=>item.listing_id===props.listing.id);const latest=outcomes[0];return <><LegacyListingDetail {...props}/><section className="listing-outcome-dock" aria-label="Marketplace outcome"><span><small>{latest?'Recorded outcome':'Close the loop'}</small><b>{latest?String(latest.outcome).replaceAll('_',' '):'What happened with this find?'}</b>{latest&&<em>{latest.final_price!=null?money(latest.final_price):latest.reason||relative(latest.occurred_at)}</em>}</span><button data-testid="record-listing-outcome" onClick={()=>props.setModal('outcome')}>{latest?'Add another outcome':'Record outcome'}</button></section></>}
...
function ChangeSetCard({changeSet,act}:{changeSet:Row;act:any}){
  const changes=Array.isArray(changeSet.changes)?changeSet.changes:[]; const [selected,setSelected]=useState<number[]>(changes.map((_:Row,index:number)=>index));
  ...
  return <article className={`change-set-card ${changeSet.status}`} data-testid={`change-set-${changeSet.id}`}><header><div><small>{changeSet.created_by_name} proposed · version {changeSet.version}</small><h3>{changeSet.title}</h3></div><em>{changeSet.status}</em></header><p>{changeSet.summary}</p><div className="change-set-items">{changes.map((change:Row,index:number)=><label key={`${changeSet.id}-${index}`}><input type="checkbox" checked={selected.includes(index)} disabled={changeSet.status!=='proposed'} onChange={()=>toggle(index)}/><span><b>{String(change.action).replaceAll('_',' ')}</b><small>{change.rationale||'Review the structured inputs before applying.'}</small></span></label>)}</div>{(changeSet.sources??[]).length>0&&<small className="change-set-sources">{changeSet.sources.length} cited source{changeSet.sources.length===1?'':'s'}</small>}<footer><button disabled={!['proposed','stale'].includes(changeSet.status)} onClick={()=>act('discard_change_set',{changeSetId:changeSet.id,expectedVersion:changeSet.version},'Proposal discarded.')}>Discard</button><button className="primary-action" data-testid={`apply-change-set-${changeSet.id}`} disabled={changeSet.status!=='proposed'||selected.length===0} onClick={()=>act('apply_change_set',{changeSetId:changeSet.id,selectedIndexes:selected,expectedVersion:changeSet.version},'Selected proposal changes applied.')}>Apply {selected.length} selected</button></footer></article>
}

function ProposalQueue({state,act}:{state:State;act:any}){const proposals=state.changeSets.filter(item=>['proposed','stale'].includes(item.status));if(!proposals.length)return null;return <section className="proposal-queue"><header><div><small>Reviewable agent work</small><h2>{proposals.length} versioned proposal{proposals.length===1?'':'s'}</h2></div><span>Choose individual changes. Nothing is applied until you decide.</span></header><div>{proposals.map(changeSet=><ChangeSetCard key={changeSet.id} changeSet={changeSet} act={act}/>)}</div></section>}

function LegacyWorkbenchView({state,selectedSessionId,selectSession,act,review}:{state:State;selectedSessionId:string;selectSession:(id:string)=>void;act:any;review:(confirmation:Row)=>void}){
...
function WorkbenchView(props:{state:State;selectedSessionId:string;selectSession:(id:string)=>void;act:any;review:(confirmation:Row)=>void}){return <><ProposalQueue state={props.state} act={props.act}/><LegacyWorkbenchView {...props}/></>}
...
function AgentView({state,act,setModal}:{state:State;act:any;setModal:(v:ModalType)=>void}){
  const [copied,setCopied]=useState(false); const skill=`Open ${typeof window==='undefined'?'/for-agents':publicAgentGuideUrl(window.location.origin)} and follow its WebMCP instructions. Use the person’s current signed-in browser session for safe workspace work, rediscover tools after authentication or capability changes, and leave every publish, message, offer, invitation, notification, or payment for exact human approval.`;
  ...
  return <><ViewHeader eyebrow="External agent access" title="Your agents, your models" copy="Zingposts has no built-in AI. A selected outside agent can use safe tools in your signed-in browser; independent access is optional." action="Add unattended agent" onAction={()=>setModal('pair')}/><section className="connection-principle"><div><span className="agent-orb">A</span><p className="eyebrow">Two trust lanes</p><h2>Seamless collaboration inside. Verified action at the marketplace edge.</h2></div><div className="connection-steps"><span><b>1</b><small>Open</small>Agent reads one URL</span><span><b>2</b><small>Sign in</small>Workspace tools appear</span><span><b>3</b><small>Collaborate</small>Browse, organize, research, draft</span><span><b>4</b><small>Approve</small>Human confirms outbound actions</span></div><p>WebMCP registration belongs to the current application document. In your signed-in browser, agent calls are attributed as browser-delegated work without another login. A separate revocable agent session is only needed for unattended or separate-browser work. {state.verification.status==='verified'?'This account is verified for publishing and contact.':'Account verification will appear only when publishing or contacting another person is requested.'}</p><button data-testid="copy-agent-skill" onClick={copySkill}>{copied?'Reconnect instruction copied':'Copy reconnect instruction'}</button></section><div className="agent-layout"><section className="agent-connections"><h2>Independent access profiles</h2>{state.agents.map(agent=><article key={agent.id}><span className="agent-orb">A</span><div><b>{agent.name}</b><small>{agent.status} · safe workspace · {(agent.scopes??[]).length} scopes</small><div className="scope-row">{(agent.scopes??[]).map((scope:string)=><span key={scope}>{scope}</span>)}</div></div>{agent.status==='pending'?<button data-testid={`approve-${agent.id}`} onClick={()=>act('approve_agent_pairing',{agentId:agent.id},'Additional scopes approved.')}>Review extra scopes</button>:agent.status==='active'?<button onClick={()=>act('revoke_agent',{agentId:agent.id},'Agent access revoked.')}>Revoke</button>:null}</article>)}{!state.agents.length&&<div className="empty"><b>No independent sessions</b><p>That is normal. Agents in this signed-in browser already have safe workspace access. Add a profile only for separate-browser or unattended work.</p></div>}</section><section className="tool-catalog"><h2>WebMCP map <span>{toolDefinitions.length}</span></h2><p className="catalog-intro"><code>get_webmcp_manifest</code> gives an outside agent this grouped map plus recommended entry points and workflows.</p>{capabilityGroups.map(([label,tools])=><section className="tool-group" key={label}><h3>{label}<span>{tools.length}</span></h3>{tools.map(([name,description])=><details key={name}><summary><code>{name}</code><span>›</span></summary><p>{description}</p></details>)}</section>)}</section></div></>}
