...
const toolDefinitions=[
  ['get_agent_bootstrap','Start or resume safely with authentication, canonical agent identity, workspace overview, attention queue, active collaboration, capabilities, and guide version.',{}],
...
function WebMCPToolGuide({pageContext,activeCapability,authenticated,status}:{pageContext:string;activeCapability:string;authenticated:boolean;status:WebMCPRuntimeStatus}){
  ...
  return <aside className={`webmcp-page-guide ${open?'open':''}`} aria-label="WebMCP tools for this page">
    {open&&<section className="webmcp-page-panel" role="dialog" aria-modal="false" aria-label={`WebMCP tools for ${current.label}`}>
      ...
      <section className="webmcp-auth-state"><b>{authenticated?'Signed in workspace':'Public, signed-out session'}</b><span>{authenticated?`${status.availableTools} active here · ${toolDefinitions.length} in the complete catalog.`:`${status.availableTools} focused public tools are active · ${publicToolNames.size} exist across public capability packs · ${signedOutCount} unlock after sign-in.`}</span></section>
      <section className="webmcp-progressive"><b>Persistent core + focused capability</b><p>Core tools stay stable. An agent activates one focused business pack without moving the person’s page; navigation is only for human review.</p><span><code>get_agent_bootstrap</code><code>activate_capability</code><code>open_for_human_review</code></span></section>
      <p><b>Active: {current.label}.</b> {current.copy}{visible.id!==current.id?` The person is currently viewing ${visible.label}; this does not change the agent’s active pack.`:''}</p>
      ...
      <details className="webmcp-other-pages"><summary><span>Other capability packs</span><b>{toolDefinitions.length-status.availableTools} inactive</b></summary><div>{others.map(group=><details key={group.id}><summary><span>{group.label}</span><b>{group.tools.length}</b></summary><div>{group.tools.map(name=><code key={name} className={!authenticated&&!publicToolNames.has(name)?'locked':'inactive-context'}>{name}</code>)}</div></details>)}</div></details>
      <footer><span>Use <code>activate_capability</code> to switch tools and <code>open_for_human_review</code> only when the person should see something.</span><a href="/for-agents">Open agent guide</a></footer>
    </section>}
...
function AgentView({state,act,setModal}:{state:State;act:any;setModal:(v:ModalType)=>void}){
  const [copied,setCopied]=useState(false); const skill=`# Zingposts operator\n\n- Read ${typeof window==='undefined'?'/for-agents':publicAgentGuideUrl(window.location.origin)} and rediscover WebMCP tools on every visit.\n- If tools are missing, distinguish browser WebMCP support from agent runtime or tab permission. Tell the person clearly and do not pretend to be connected.\n- Do not replace unavailable WebMCP tools with DOM automation for marketplace changes.\n- Start with get_agent_bootstrap.\n- Call activate_capability for the focused business pack; it does not navigate. Use open_for_human_review only when the person should see a record.\n- Treat Zingposts boards, alerts, research, drafts, versions, and activity as durable state.\n- Refresh and retry from currentVersion after a CONFLICT.\n- Never bypass account verification or human confirmation for publishing, messages, offers, invitations, notifications, or payments.\n- Never store invite fragments, fallback codes, or a stale tool catalog.`;
