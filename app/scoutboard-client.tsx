...
function WorkspaceSidebar({state,webmcpStatus,view,query,setQuery,category,viewMode,filtersExpanded,draggingId,collapsed,accountOpen,setAccountOpen,nav,toggle,setModal,signOut,setMarketplaceView,onDrop}:{state:State;webmcpStatus:WebMCPRuntimeStatus;view:string;query:string;setQuery:(value:string)=>void;category:string;viewMode:string;filtersExpanded:boolean;draggingId:string|null;collapsed:boolean;accountOpen:boolean;setAccountOpen:(value:boolean)=>void;nav:(value:string)=>void;toggle:()=>void;setModal:(value:ModalType)=>void;signOut:()=>void;setMarketplaceView:(value:Row)=>void;onDrop:(boardId:string,listingId:string)=>void}){
  ...
  const copyAgentInvite=async()=>{try{const copied=await writeClipboardText(publicAgentGuideUrl(window.location.origin));setInviteCopyState(copied?'copied':'failed');}catch{setInviteCopyState('failed');}setTimeout(()=>setInviteCopyState('idle'),2600)};
...
configs.outcome={title:'Record what happened',copy:'Keep the result private and attributable so you and your outside agent can use real history later.',fields:[['outcome','Outcome','text','purchased, sold, passed, expired, unresponsive, not_as_described, or other'],['finalPrice','Final price','number',String(listing?.price??'')],['reason','Primary reason','text','Condition, price, timing, or seller response'],['notes','Notes','textarea','What should your agent remember next time?']]};
