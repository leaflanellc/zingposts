...
const WEBMCP_PAGE_GROUPS=[
  ...
  {id:'activity',label:'Activity & QA',copy:'Inspect attributable work, reverse reversible actions, and manage isolated test artifacts.',tools:['list_recent_agent_actions','undo_agent_action','start_qa_run','preview_qa_cleanup','request_qa_cleanup']},
...
const TOOL_FIELD_ENUMS:Record<string,Record<string,string[]>>={save_listing:{status:['watching','researching','inspection','negotiating','archived']},set_listing_status:{status:['watching','researching','inspection','negotiating','archived']},update_negotiation_status:{status:['active','waiting_buyer','waiting_seller','paused','closed']},record_trade_decision:{decision:['draft','reviewing','ready_for_invites','active','complete','declined','cancelled']},respond_to_collaboration_item:{decision:['answered','accepted','declined','deferred']}};
...
const CONSEQUENTIAL_TOOLS=new Set(['request_listing_publish','attach_listing_image_from_url','enable_alert','request_message_send','request_offer_submit','respond_to_offer','request_trade_invitation','request_qa_cleanup']);
const DESTRUCTIVE_TOOLS=new Set(['revoke_agent','archive_listing','unsave_listing','remove_listings_from_board','pause_alert','request_qa_cleanup']);
