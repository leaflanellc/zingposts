...
export async function GET(request:Request){
  const url=new URL(request.url); const code=url.searchParams.get('code'); const destination=new URL('/',publicSiteOrigin(request));
  if(!code){console.warn(JSON.stringify({event:'auth_callback_incomplete'}));destination.searchParams.set('auth_error','The sign-in link is incomplete. Request a new email.');return NextResponse.redirect(destination)}
  try{
