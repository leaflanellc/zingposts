...
assert.equal(manifest.progressiveDiscovery.currentContext, 'marketplace');
...
const capabilityIndex = await action('get_capability_index', { currentContext: 'marketplace' });
assert.equal(capabilityIndex.strategy, 'progressive_page_context');
assert.equal(capabilityIndex.currentContext, 'marketplace');
assert.ok(capabilityIndex.frontDoorTools.includes('navigate_to_workspace'));
assert.ok(capabilityIndex.groups.some((group) => group.id === 'listing' && group.entryTools.includes('get_price_research')));
const priceCapability = await action('get_capability_group', { capabilityId: 'listing' });
assert.ok(priceCapability.tools.includes('analyze_price_and_offer'));
assert.deepEqual(priceCapability.requiredContext, ['listingId']);
const listingNavigation = await action('navigate_to_workspace', { capabilityId: 'listing', listingId: 'lst_whaler', section: 'research' });
assert.equal(listingNavigation.path, '/listings/lst_whaler#research');
assert.equal(listingNavigation.rediscoverTools, true);
const publicListingNavigation = await publicAction('navigate_to_workspace', { capabilityId: 'listing', listingId: 'lst_whaler' });
assert.equal(publicListingNavigation.detailOpen, true);
